Public Class ProjectForm
    ...
    Private Sub DeleteButton_Click(sender As Object, e As EventArgs) Handles DeleteButton.Click
        '削除処理の実行を確認する
        If MsgBox("このプロジェクトを削除してもよろしいですか？", MsgBoxStyle.YesNo Or        MsgBoxStyle.DefaultButton2) <> MsgBoxResult.Yes Then
            Return
        End If
        ...
        Using connection As New SqlClient.SqlConnection(
            ...
            Using command As SqlClient.SqlCommand = connection.CreateCommand()
                ...
                Try
                    ...
                    command.CommandText = "DELETE FROM tbl_project WHERE project_code='" & Me.Project_codeTextBox.Text & "'"
                    command.ExecuteNonQuery()

                    'tbl_jobテーブルのデータ削除
                    command.CommandText = "DELETE FROM tbl_job WHERE project_code='" & Me.Project_codeTextBox.Text & "'"
                    command.ExecuteNonQuery()
    ...
    Private Sub JobButton_Click(sender As Object, e As EventArgs) Handles JobButton.Click
        Dim frm As New JobForm '［ジョブ管理］フォーム

        '［ジョブ管理］フォームのSetProjectCodeプロシージャを呼び出して、
        '指定したプロジェクトコードのデータを表示する
        frm.SetProjectCode(Project_codeTextBox.Text)
        '［ジョブ管理］フォームを表示する
        frm.ShowDialog(Me)

        '［ジョブ管理］フォームから戻ってきたら、ジョブ一覧を更新する
        Me.Tbl_jobTableAdapter.FillByPcode(Me.Project_jobDataSet.tbl_job, Project_codeTextBox.Text)
