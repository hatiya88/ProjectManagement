Public Class StaffConvertForm
    Private Sub StaffConvertForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim sql As String
        ...
        Using connection As New SqlClient.SqlConnection(
            ...
            Using command As New SqlClient.SqlCommand(sql, connection)
                ...
                Do While dr.Read
                    StaffCodeBeforeComboBox.Items.Add(dr("staff_code") & ":" & dr("staff_name"))
                    StaffCodeAfterComboBox.Items.Add(dr("staff_code") & ":" & dr("staff_name"))
                Loop
    ...
    Private Sub ExecuteButton_Click(sender As Object, e As EventArgs) Handles ExecuteButton.Click
        ...
        If StaffCodeBeforeComboBox.Text = "" Then
            MsgBox("変更前のスタッフを指定してください")
            Return
        End If

        If StaffCodeAfterComboBox.Text = "" Then
            MsgBox("変更後のスタッフを指定してください")
            Return
        End If

        'スタッフコードの取得（先頭6文字）
        stcode_before = Strings.Left(StaffCodeBeforeComboBox.Text, 6)
        stcode_after = Strings.Left(StaffCodeAfterComboBox.Text, 6)
